...
while True:
    ...
    for tile in tiles:
        ...
        if self.GameHelper.compare_total_units(game.myId, game.eId) & self.GameHelper.compare_building_count(
                game.myId, game.eId) & self.GameHelper.compare_resource(game.myId, game.eId):

            (nearest_enemy_building_position,
             distance) = self.GameHelper.get_nearest_building_position_and_distance_belonging_to_player(
                tile.position[0], tile.position[1], playerId)
            ...
            if (self.GameHelper.get_nearest_building_position_and_distance_belonging_to_player(
                tile.position[0], tile.position[1], playerId) is not None) & self.GameHelper.get_tile(nearest_enemy_building_position[0],
                                        nearest_enemy_building_position[1]).building.defense < tile.units[playerId]:

                commands.append(self.GameHelper.single_move_towards_tile_avoiding_things(playerId, tile.position,
                                                                                         nearest_enemy_building_position,
                                                                                         tile.units[playerId],
                                                                                         'stronger_buildings'))
            else:
